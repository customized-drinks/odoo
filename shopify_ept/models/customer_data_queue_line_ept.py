...
class ShopifyCustomerDataQueueLineEpt(models.Model):
    ...
    def shopify_customer_data_queue_line_create(self, result, customer_queue_id):
        ...
        existing_customer_data_queue_line = synced_shopify_customers_line_obj.search(
            [('shopify_customer_data_id', '=', customer_id),
             ('shopify_instance_id', '=', customer_queue_id.shopify_instance_id.id),
             ('state', 'in', ['draft', 'failed'])])
        existing_customer_queue = existing_customer_data_queue_line.synced_customer_queue_id
        line_vals = {
        ...
        if not existing_customer_data_queue_line:
            ...
            existing_customer_data_queue_line.update(line_vals)
