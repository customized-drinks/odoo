...
class DHLPaketProvider():
    ...
    def _get_acc_no(self, carrier):
        product_procedure_dict = {
            ...
            'V66WPI': '66'  # DHL Warenpost International
        }
    ...
    def _get_services(self, picking, carrier, package_details):
        ...
        # if carrier.dhl_de_paket_packaging_return:
        #     PackagingReturn = self.client.factory.create('PackagingReturn')
        #     service.PackagingReturn = PackagingReturn
        #     service.PackagingReturn._active = 1
    ...
    def _get_shipment_details(self, picking, carrier, shipment_item):
        ...
        if carrier.return_label_on_delivery:
            shipmentDetails.returnShipmentAccountNumber = carrier.dhl_de_paket_return_account_number
            retref = "%s:RETOURE" % shipment_item['name']
            shipmentDetails.returnShipmentReference = retref[:35]
    ...
    def _get_shipper(self, picking, carrier):
        ...
        shipper_partner = carrier.dhl_de_paket_shipper_id or False
    ...
    def _get_receiver_packstation(self, partner):
        ...
        packstationnumber, postnumber  = packstation_details.strip().split(' ')
    ...
    def _get_receiver(self, picking, carrier):
        receiver = self.client.factory.create('ReceiverTypeType')
    ...
    def create_shipment(self, picking, carrier):
        ...
        create_shipment_args = dict(labelResponseType=labelresponsetype,
                                    labelFormat=labelformat)
        if carrier.return_label_on_delivery:
            create_shipment_args.update({'labelFormatRetoure': labelformat})
        # groupProfileName
        # labelFormat
        # labelFormatRetoure
        # combinedPrinting
        ...
        response = self.client.service.createShipmentOrder(version, shipmentOrders, **create_shipment_args)
        return response
    ...
    def get_label(self, picking, carrier, tracking_number):
        version = self._get_version()
        labelresponsetype = carrier.dhl_de_paket_label_response_type or 'B64'  # URL,    B64,    ZPL2
        labelformat = carrier.dhl_de_paket_label_format or 'A4'
        create_shipment_args = dict(labelResponseType=labelresponsetype,
                                    labelFormatRetoure=labelformat)
        # groupProfileName
        # labelFormat
        # labelFormatRetoure
        # combinedPrinting
        response = self.client.service.getLabel(version, tracking_number, **create_shipment_args)
        return response
