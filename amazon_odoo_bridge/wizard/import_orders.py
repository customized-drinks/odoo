...
class ImportMwsOrders(models.TransientModel):
    ...
    _description = "Import Mws Orders"
    ...
    def _wk_list_orders(self, **kwargs):
        ...
        if context.get("order_import_cron"):
            create_after = fields.Datetime.from_string(
                channel_id.import_order_date or self.from_date)
        elif context.get("order_status_cron"):
            create_after = fields.Datetime.from_string(
                channel_id.update_order_date or self.from_date)
        else:
            create_after = fields.Datetime.from_string(
                self.from_date)
        try:
            create_after += relativedelta(days=-1)
    ...
    def _wk_list_order_items(self,order_id):
        for i in [5, 10, 20, 50, 120]:
            ...
            try:
                ...
                _logger.error("##%s"%(message))
                res['message']=message
                if me.response.status_code == 503:
                    time.sleep(i)
                    _logger.error(f'sleep {i}')
                else:
                    break
            except Exception as e:
    ...
    def import_amazon_orders(self,channel_id,create_ids=[],update_ids=[],message='',nextToken=None):
        ...
        if not orders:
            ...
            if self._context.get("order_import_cron") and last_order_date:
                import_order_date = str(date_parse(last_order_date)).split('.')[0]
                # channel_id.import_order_date = fields.Datetime.to_datetime(import_order_date) + relativedelta(day=-1)
                channel_id.import_order_date = import_order_date
