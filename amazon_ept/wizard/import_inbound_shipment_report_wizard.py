...
class AmazonInboundShipmentReportWizard(models.TransientModel):
    ...
    def get_file_name(self, name=datetime.strftime(datetime.now(), '%Y%m%d%H%M%S%f')):
        """
        This method will return imported inbound shipment file name.
        :param name: string : imported shipment name
        :return: will return the inbound shipment csv file name
        """
    ...
    def fill_dictionary_from_file(reader):
        ...
        for row in reader:
            vals = {
                ...
                'dimension_type': row.get('Dimension Type', '').casefold(),
                'dimension_unit': row.get('Dimension Unit', '').casefold(),
    ...
    def import_inbound_shipment_report(self):
        """
        This method will Import inbound shipment excel report.
        @param: {}
        @return: {}
        """
        ...
        if self.check_fields_validation(fields_name):
            ...
            for row in reader:
                ...
                _dimension_domain = [
                    ("type", "=ilike", row.get('Dimension Type', '').casefold()),
                    ("dimension_unit", "=ilike", row.get('Dimension Unit', '').casefold()),
                ...
                if not product_ul:
                    dimension_vals = {
                        ...
                        'type': row.get('Dimension Type', '').casefold(),
                        'dimension_unit': row.get('Dimension Unit', '').casefold(),
                        'height': row.get('Height', 0.00),
                        'width': row.get('Width', 0.00),
                        'length': row.get('Length', 0.00),
