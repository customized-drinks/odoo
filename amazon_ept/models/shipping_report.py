...
class ShippingReportRequestHistory(models.Model):
    ...
    mismatch_details = fields.Boolean(compute='_compute_total_logs', string='Mismatch Details',
                                      help='True if found mismatch details in log line')

    def unlink(self):
        """
        This method will raise UserError when processed or partially processed
        report deleted .
        :return:
        """
        for report in self:
    ...
    def _check_duration(self):
        """
        Compare Start date and End date, If End date is before start date raise warning.
        @author: Keyur Kanani
    ...
    def default_get(self, fields):
        ...
        @author: Keyur Kanani
        :param fields:
        :return: dict {} - values of shipping.report.request.history() model record
        """
    ...
    def request_report(self):
        """
        Request GET_AMAZON_FULFILLED_SHIPMENTS_DATA_GENERAL Report from Amazon for specific date range.
        @author: Keyur Kanani
        :return: Boolean
        """
    ...
    def update_report_history(self, request_result):
        ...
        document_id = str(request_result.get('reportDocumentId', ''))
        vals = {}
        if not self.report_document_id and document_id:
            vals.update({'report_document_id': document_id})
        if report_state:
    ...
    def amz_search_or_create_logs_ept(self, message):
        """
        This method will create log book record if not already exist log book record for selected
        shipping.report.request.history model record.
        :param message: message for log lines
        ...
        if message:
            common_log_line_obj.amazon_create_order_log_line(
                message, model_id, self.id, False, False, 'FBA', log_rec)
        return log_rec
    ...
    def process_shipment_file(self):
        ...
        for row in reader:
            ...
            if row.get('merchant-order-id', False):
                ...
                if row.get('amazon-order-id', False) and row.get('amazon-order-id', False) not in sale_order_list:
                    sale_order_list.append(row.get('amazon-order-id', False))
    ...
    def get_amazon_product(self, instance, sku, log_rec):
        ...
        if not amz_product:
            ...
            self.env[COMMON_LOG_LINES_EPT].amazon_create_product_log_line(log_line_msg, model_id, False, sku,
                                                                          'FBA', log_rec, mismatch=True)
        return amz_product
    ...
    def get_shipment_details(dict_val):
        ...
        """
        return (
            dict_val.get('shipment-id', ''), dict_val.get('warehouse', False),
            float(dict_val.get('quantity-shipped', 0.0)),
    ...
    #             self.env[COMMON_LOG_LINES_EPT].amazon_create_common_log_line_ept(skip_reason, model_id, self.id,
    ...
    #     move = move.filtered(lambda m: m.picking_id.state not in ['done',
    #                                                               'cancel'] and m.picking_id.location_dest_id.usage == 'customer')
    ...
    #         move = line.move_ids.filtered(lambda
    #                                           m: m.state == 'done' and m.picking_id.location_dest_id.usage == 'customer' and not m.amazon_shipment_id)
    ...
    #                                 line.copy(
    #                                     default={'order_id': new_order.id, 'product_uom_qty': line.product_uom_qty,
    #                                              **amz_extra_vals})
    ...
    #         if not order_lines:
    #             skip_reason = 'Skipped an order named {} because order lines with non service type products were not found.'.format(
    #                 amz_order.name)
    #             self.env[COMMON_LOG_LINES_EPT].amazon_create_common_log_line_ept(skip_reason, model_id, self.id,
    #                                                                              log_rec, mismatch=True)
    #             continue
    ...
    #             self.env[COMMON_LOG_LINES_EPT].amazon_create_common_log_line_ept(ex, model_id, self.id, log_rec)
    ...
    #             lambda ol: ol.product_id.id == product.id and ol.order_id.warehouse_id.id == fc_warehouse)
    #         move = line.move_ids.filtered(
    #             lambda m: m.state == 'done' and shipment_id in m.amazon_shipment_id.split(','))
    #         move.fulfillment_center_id = val.get('fulfillment_center', '')
    #         move.filtered(lambda m: m.state not in ['done', 'cancel'])._action_assign()
    #         carrier_id = False
    #         if val.get('carrier', ''):
    #             carrier_id = self.env[SALE_ORDER].get_amz_shipping_method(val.get('carrier', ''),
    #                                                                       line.order_id.amz_seller_id.shipment_charge_product_id)
    ...
    def validate_stock_move(move, job, order_name, process_invoice):
        """
        This method will either validate stock move or crate log lines if stock move not validate.
        :param move: stock.move() object
        :param job: common.log.boob.ept() object
        :param order_name: string - amazon order reference
    ...
    @staticmethod
    def amz_create_and_process_fba_invoices(order, process_invoice):
        """
        ...
        for order_ref, lines in outbound_order_data.items():
            ...
            for shipment_id, shipment_lines in shipment_dict.items():
                ...
                for ship_line in shipment_lines:
                    if ship_line.get('tracking-number', False) and ship_line.get('tracking-number',
                                                                                 False) not in track_list:
                        track_list.append(ship_line.get('tracking-number', False))
                        track_num = ship_line.get('tracking-number',
                                                  False) if not track_num else track_num + ',' + ship_line.get(
                ...
                for product, shipped_qty in prod_dict.items():
                    ...
                    sm_vals = self.prepare_outbound_stock_move_update_vals(shipment_id, amazon_order, ship_line,
                                                                           track_num)
    ...
    #         if data.get('carrier', ''):
    #             # shipment_charge_product_id is fetched according to seller wise
    #             carrier_id = self.env[SALE_ORDER].get_amz_shipping_method(data.get('carrier', ''),
    ...
    def process_fba_shipment_orders(self, order_details_dict_list, amz_b2b_order_dict, job, sale_orders_list):
        ...
        for order_ref, lines in order_details_dict_list.items():
            ...
            for order_line in lines:
                ...
                so_lines = sale_order_line_obj.create_amazon_sale_order_line(
                    amazon_order, order_line, product_details)
    ...
    def process_shipment_report_stock_move_ept(self, stock_move_vals):
        """
        This method will create and process stock move for FBA orders.
    ...
    def search_or_create_partner(self, row, instance, partner_dict, country_dict, state_dict):
        ...
        if not ship_state and ship_country and row.get('ship-state', '') != '--':
            ship_state = res_partner_obj.create_or_update_state_ept(ship_country.code, row.get('ship-state', ''),
            ...
            if not amazon_product:
                ...
                if odoo_product:
                    ...
                    common_log_line_obj.amazon_create_product_log_line(
                        message, model_id, odoo_product, seller_sku, 'FBA', job)
    ...
    def auto_import_shipment_report(self, args):
        """
        This method will import shipment report by using scheduler.
        :param args: dict {}
        :return: boolean(TRUE/FALSE)
        """
            ...
            if not seller.is_another_soft_create_fba_shipment:
                ...
                for report in list_of_wrapper:
                    ...
                    if not self.search([('report_id', '=', report_id),
                                        ('report_document_id', '=', request_id),
                                        ('report_type', '=', report_type)]):
                        start = parser.parse(str(report.get('dataStartTime', ''))).astimezone(utc).strftime('%Y-%m-%d %H:%M:%S')
                        end = parser.parse(str(report.get('dataEndTime', ''))).astimezone(utc).strftime('%Y-%m-%d %H:%M:%S')
                        self.create({
                            'seller_id': seller_id,
    ...
    def auto_process_shipment_report(self, args={}):
        """
        This method will process shipment report by using scheduler.
        :param args: dict {}
        :return: boolean(True/False)
        """
        seller_id = args.get('seller_id', False)
    ...
    def get_reports_from_other_softwares(self, seller, report_type, start_date, end_date):
        """
        This method will get reports which created by other software in amazon.
        ...
        """
        kwargs = self.prepare_amazon_request_report_kwargs(seller)
        kwargs.update({'emipro_api': 'get_reports_sp_api', 'report_type': [report_type],
                       'start_date': start_date, 'end_date': end_date})
        response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT + '/iap_request', params=kwargs, timeout=1000)
    ...
    def get_missing_fulfillment_center(self, attachment_id):
        ...
        for fulfillment_center in fulfillment_center_list:
            amz_fulfillment_center_id = fulfillment_center_obj.search(
                [('center_code', '=', fulfillment_center), ('seller_id', '=', seller_id)])
    ...
    def decode_amazon_encrypted_attachments_data(self, attachment_id, job=False):
        dbuuid = self.env['ir.config_parameter'].sudo().get_param('database.uuid')
        ...
        response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT + '/decode_data', params=kwargs, timeout=1000)
        ...
        elif self._context.get('is_auto_process', False) and job:
