...
class StockPicking(models.Model):
    ...
    def check_amazon_shipment_status_ept(self, items, job_id):
        ...
        :param items: list amazon response
        :param job_id: common.log.book.ept() objet
        ...
        for picking in pickings:
            ...
            for item in items:
                ...
                inbound_shipment_plan_line = picking.odoo_shipment_id.odoo_shipment_line_ids.filtered(
                ...
                move_lines = picking.move_lines.filtered(
                    lambda move_lines, odoo_product_id=odoo_product_id: move_lines.product_id.id == odoo_product_id and move_lines.state not in
                                                                        ('draft', 'done', 'cancel', 'waiting'))
                if not move_lines:
                    move_lines = picking.move_lines.filtered(
                        lambda move_line, odoo_product_id=odoo_product_id: move_line.product_id.id == odoo_product_id and move_line.state not in
                                                                           ('draft', 'done', 'cancel'))
                ...
                if qty_left > 0.0 and move.product_id.tracking in ['lot', 'serial']:
                    self.amz_create_tracking_product_stock_move_line(amazon_product.product_id, picking, qty_left, move)
                if qty_left > 0.0 and move.product_id.tracking == 'none':
                    stock_move_line_obj.create({
                        'product_id': amazon_product.product_id.id,
                        'product_uom_id': amazon_product.product_id.uom_id.id,
                        'picking_id': picking.id,
    ...
    def check_qty_difference_and_create_return_picking_ept(self, amazon_shipment_id, odoo_shipment_id,
                                                           instance, items):
        """
        This method will create return picking and confirm and process that.
        :param amazon_shipment_id: amazon shipment id
        :param odoo_shipment_id: odoo shipment id record
        :param instance: amazon.instance.ept() obejct
        :param items: list of amazon shipment items
        :return: boolean(TRUE/FALSE)
        """
        ...
        for item in items:
            ...
            done_moves = odoo_shipment_id.picking_ids.filtered(
                ...
                'move_lines').filtered(
                lambda r, amazon_product=amazon_product: r.product_id.id == amazon_product.product_id.id and r.state == 'done' and r.location_id.id == location_id and r.location_dest_id.id == location_dest_id)
            ...
            if received_qty < 0.0:
                return_moves = odoo_shipment_id.picking_ids.filtered(
                    lambda r: r.is_fba_wh_picking and r.amazon_shipment_id == amazon_shipment_id).mapped(
                    'move_lines').filtered(
                    lambda r, amazon_product=amazon_product: r.product_id.id == amazon_product.product_id.id and r.state == 'done' and r.location_id.id == location_dest_id and r.location_dest_id.id == location_id)
        ...
        if return_picking:
            ...
            self.env['stock.immediate.transfer'].with_context(default_pick_ids=[(4, return_picking.id)],
    ...
    def check_amazon_shipment_status(self, response):
        """
        This method will Check Shipment Status from Amazon.
        :param response: shipment status response from amazon
        :return: boolean(TRUE/FALSE)
        """
        amazon_shipment_ids = []
        for picking in self:
    ...
    def amz_create_tracking_product_stock_move_line(self, product_id, picking, qty_left, move):
        """
        This method will set lot number in stock move line if not set already and if required
        than create stock move line for tracking lot or serial type products.
        :param product_id: product.product() object
        :param picking: stock.picking() object
        :param qty_left: qty for create stock move lines
        :param move: stock.move() object
        :return: boolean(TRUE/FALSE)
        """
        stock_move_line_obj = self.env['stock.move.line']
        assign_move_lines = move.move_line_ids.filtered(
            lambda move_line: move_line.product_id.id == product_id.id
                              and move_line.state in ['done', 'assigned', 'partially_available'])
        for move_line in assign_move_lines:
            if not move_line.lot_id:
                lot_id = self.amz_get_lot_number_for_stock_move_line(product_id, move_line, move)
                move_line.write({'lot_id': lot_id.id if lot_id else False})

        if assign_move_lines and sum(assign_move_lines.mapped('qty_done')) < qty_left:
            remaining_qty = abs(qty_left - sum(move.move_line_ids.mapped('qty_done')))
            sml_vals = self.amz_prepare_stock_move_line(product_id, picking, remaining_qty, move)
            new_move_line = stock_move_line_obj.create(sml_vals)
            lot_id = self.amz_get_lot_number_for_stock_move_line(product_id, new_move_line, move)
            new_move_line.update({'lot_id': lot_id.id if lot_id else False})
        return True

    @staticmethod
    def amz_get_lot_number_for_stock_move_line(product_id, move_line, move):
        """
        This method will return appropriate lot number for stock move line.
        :param product_id: product.product() object
        :param move: stock.move() object
        :return: stock.production.lot() object or FALSE
        """
        for lot_id in move.lot_ids:
            available_qty = product_id.with_context(location=move.location_id.id, lot_id=lot_id.id).qty_available
            if available_qty >= move_line.qty_done:
                if move.move_line_ids.filtered(lambda move_line: move_line.lot_id.id == lot_id.id):
                    continue
                return lot_id
        return move.lot_ids[0] if move.lot_ids else False
    ...
    def amz_inbound_get_stock_move_ept(picking, odoo_product_id):
        ...
        if not stock_move:
            stock_move = picking.move_lines.filtered(
                lambda r: r.product_id.id == odoo_product_id.id and r.state not in ('draft', 'done', 'cancel'))
    ...
    def amz_inbound_shipment_plan_line_ept(self, odoo_shipment_id, amazon_product, item):
        ...
        received_qty = float(item.get('QuantityReceived', 0.0))
        inbound_shipment_plan_line_id = odoo_shipment_id.odoo_shipment_line_ids.filtered(
    ...
    def check_qty_difference_and_create_return_picking(self, response, amazon_shipment_id, odoo_shipment_id,
        ...
        :param response: Amazon Shipment Response
        :param amazon_shipment_id: Amazon Shipment Reference
        :param odoo_shipment_id: Odoo Shipment Reference
        :param instance: amazon.instance.ept()
        :return: boolean or stock.picking() object
        """
        amazon_product_obj = self.env['amazon.product.ept']
        return_picking = False
        pick_type_id = False
        ...
        for item in response.get('items', {}):
            ...
            done_moves = shipment_pickings.mapped('move_lines').filtered(
                lambda r, amazon_product=amazon_product: r.state == 'done' and r.product_id.id == amazon_product.product_id.id and r.location_dest_id.id == location_dest_id and r.location_id.id == location_id)
            if received_qty <= 0.0 and (not done_moves):
                continue
            for done_move in done_moves:
                received_qty = received_qty - done_move.product_qty
            if received_qty < 0.0:
                ...
                for move in done_moves:
                    ...
                    amz_return_move = self.amz_copy_stock_move_data_ept(move, return_picking, received_qty, pickings,
                                                                        pick_type_id)
