...
class InboundShipmentEpt(models.Model):
    ...
    intended_box_contents_source = fields.Selection(
        ...
        help="If your instance is USA then you must set box content, "
             "other wise amazon will collect per piece fee", string="Intended BoxContents Source")
    ...
    picking_ids = fields.One2many('stock.picking', 'odoo_shipment_id', string="Pickings", readonly=True)
    ...
    amazon_shipment_weight_unit = fields.Selection(
        [('pounds', 'Pounds'), ('kilograms', 'Kilograms'), ], string='Weight Unit',
        compute="_compute_shipment_weight", store=True, readonly=True)
    ...
    def _compute_picking_count(self):
        """
        This method will count the number of pickings
        :return: N/A
        """
        for rec in self:
    ...
    def action_view_pickings(self):
        """
        This method will create and return the action to open the view of stock.picking
        :return:action
        """
        action = {
    ...
    def _compute_are_all_pickings_cancelled(self):
        """
        This method will check all pickings are cancel or not in inbound shipment.
        :return:
        """
        for record in self:
    ...
    def _compute_transport_type(self):
        """
        This method will set transport type of inbound shipment.
        :return:
        """
        for shipment in self:
    ...
    def create_or_update_inbound_shipment(self, ship_plan, shipment, job=False):
        ...
        amz_create_date = fields.datetime.now().date() if ship_plan else False
        ...
        sku_qty_list = self.env['inbound.shipment.plan.line'].prepare_shipment_plan_line_dict(odoo_shipment, items)
        address = ship_plan.ship_from_address_id
        address_dict = self.prepare_address_dict_ept(address)
        ...
        if emipro_api:
            ...
            kwargs.update({'destination': fulfill_center_id, 'emipro_api': emipro_api,
                           'marketplace_id': instance.market_place_id,
                           'sku_qty_list': sku_qty_list, 'address_dict': address_dict,
                           'labelpreppreference': label_prep_type,
                           'inbound_box_content_status': odoo_shipment.intended_box_contents_source,
                           'cases_required': is_are_cases_required, 'shipment_name': odoo_shipment.name,
                           'shipment_id': odoo_shipment.shipment_id, 'shipment_status': 'WORKING'})
            response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT + '/iap_request', params=kwargs, timeout=1000)
            if response.get('error', {}):
                job = self.search_or_create_inbound_common_log_ept('export', response.get('error', {}))
                return False, job
    ...
    @staticmethod
    def prepare_address_dict_ept(address):
        """
        This method will prepare shipping address of partner for amazon shipment.
        :param address: res.partner() object
        :return:
        """
        return {
    ...
    def create_shipment_picking(self):
        """
        This method will Import Shipment from amazon to odoo by using shipping ids, also
    ...
    def amz_find_location_routes_ept(self, warehouse):
        ...
        if not location_routes:
            ...
            self.search_or_create_inbound_common_log_ept('export', error_value)
    ...
    def amz_inbound_get_warehouse_ept(self, instance, fulfill_center):
        ...
        fulfillment_center = fulfillment_center_obj.search(
            [('center_code', '=', fulfill_center), ('seller_id', '=', instance.seller_id.id)], limit=1)
    ...
    def search_or_create_inbound_common_log_ept(self, log_type, message):
        """
        ...
        mod: Updated search condition as search the log with the message but not passed during creations.
        """
        ...
        job = log_book_obj.search([('module', '=', 'amazon_ept'), ('model_id', '=', model_id),
                                   ('res_id', '=', self.id)])
        if not job:
        ...
        log_line_obj.create({'message': message, 'model_id': model_id,
                             'res_id': self.id, 'log_book_id': job.id})
        return job

    def create_pickings_ept(self):
        """
        This method will Create New Picking for Shipment
        :return:
        """
        self.ensure_one()
        inbound_shipment_plan_obj = self.env['inbound.shipment.plan.ept']
    ...
    def update_inbound_shipment_qty(self):
        """
        This method will Update Shipment Qty in Amazon
        :return: boolean(TRUE/FALSE)
        """
    ...
    def cancel_shipment_in_amazon_via_shipment_lines(self):
        """
        This method will prepare dict of sku, quantity and request to cancel shipment in amazon.
        :return: boolean(TRUE/FALSE)
        """
        shipment = self
    ...
    def open_import_inbound_shipment_report_wizard(self):
        """
        This method will open inbound shipment report wizard form view.
    ...
    def get_instance(shipment):
        """
        This method will return the shipment instance.
        :param shipment:
    ...
    def get_header(instance):
        """
        This method will return the request header
        :param instance: amazon.instance.ept()
        ...
        for shipment in self:
            ...
            for content in parcels:
                ...
                for carton_line in content.carton_info_ids:
                    line = shipment.odoo_shipment_line_ids.filtered(
                        lambda shipment_line, carton_line=carton_line: shipment_line.amazon_product_id.id == carton_line.amazon_product_id.id)
    ...
    def setup_partnered_ltl_data(self, parcel_data):
        ...
        :param parcel_data: amazon return response
        :return: boolean(TRUE/FALSE)
        """
        self.ensure_one()
    ...
    def setup_partnered_small_parcel_data(self, parcel_data):
        ...
        for package in package_list:
            ...
            if weight_unit:
                package_vals.update({'weight_unit': weight_unit}), domain.append(('weight_unit', '=', weight_unit))
            if weight_value:
                package_vals.update({'weight_value': weight_value}), domain.append(('weight_value', '=', weight_value))
    ...
    def estimate_transport_request(self):
        ...
        instance = self.get_instance(self)
        kwargs = self.amz_prepare_inbound_shipment_kwargs_vals(instance)
        kwargs.update({'emipro_api': 'estimate_transport_request_sp_api',
    ...
    def get_carton_content_result(self):
        ...
        try:
            ...
            # need to take changes as per SP API user guide response changes.
            if response.get('error', False):
                raise UserError(_(response.get('error', {})))
            result = response.get('result', {})
    ...
    def export_partnered_small_parcel(self):
        """
        This method is used to export small parcel into amazon.
        :return: boolean(TRUE/FALSE)
        """
        log_obj = self.env['common.log.book.ept']
        log_line_obj = self.env['common.log.lines.ept']
        for shipment in self:
            ...
            if flag:
                ...
                if response.get('error', {}):
                    ...
                    transport_status = result and result.get('TransportResult', {}).get('TransportStatus', '')
                    if transport_status in transport_status_list:
                        shipment.write({'transport_state': transport_status,
    ...
    def export_non_partnered_small_parcel_tracking(self):
        ...
        for shipment in self:
            ...
            pickings = shipment.picking_ids.filtered(
                lambda pick: pick.state == 'done' and not pick.is_fba_wh_picking and not pick.updated_in_amazon)
            if not pickings:
                ...
                for move in move_lines:
                    if move.state == 'done':
                        ...
                        sku_qty_dict.update({str(
                            line and line.seller_sku or amazon_product[0].seller_sku): str(int(qty))})
                if sku_qty_dict:
            ...
            for x in range(0, len(picking.move_lines), 20):
                ...
                for move in move_lines:
                    ...
                    if str(line and line.seller_sku or amazon_product[0].seller_sku) not in sku_qty_dict:
                        sku_qty_dict.update({str(
                            line and line.seller_sku or amazon_product[0].seller_sku): str(int(0.0))})
                if sku_qty_dict:
                    for sku, qty in sku_qty_dict.items():
                        list_of_dict.append({'SellerSKU': sku, 'QuantityShipped': int(qty)})
                self.update_shipment_in_amazon(picking, list_of_dict, instance, fba_shipment, job, auto_called)
    ...
    def update_shipment_in_amazon(self, picking, sku_qty_list, instance, fba_shipment, job, auto_called):
        """
        This method will update shipment in amazon
        :param picking: stock.picking() object
        :param sku_qty_list: list []
        :param instance: amazon.instance.ept() object
        :param fba_shipment: amazon.inbound.shipment.ept() object
        :param job: common.log.book.ept() object
        :param auto_called: boolean (TRUE/FALSE)
        :return: boolean(TRUE/FALSE)
        """
        log_line_obj = self.env['common.log.lines.ept']
        ...
        kwargs.update({
            ...
            'cases_required': fba_shipment.shipment_plan_id.is_are_cases_required,
            'sku_qty_list': sku_qty_list,
            'address_dict': address_dict,
            'destination': destination
        })
        ...
        if response.get('error', {}):
            ...
            if job:
                log_line_vals = {'model_id': log_line_obj.get_model_id('amazon.inbound.shipment.ept'),
                                 'res_id': fba_shipment.id, 'message': error_value, 'log_book_id': job.id}
                log_line_obj.create(log_line_vals)
            else:
                if not auto_called:
                    raise UserError(_(error_value))
    ...
    def export_non_partnered_ltl_parcel_tracking(self):
        """
        If Data updated in Amazon then update picking record and set
    ...
    def void_transport_request(self):
        """
        We need to only reset Inbound Shipment if we get status = Voided,
    ...
    def get_package_labels(self):
        ...
        if ctx.get('label_type', '') == 'delivery':
            view = self.env.ref('amazon_ept.amazon_inbound_shipment_print_delivery_label_wizard_form_view', False)
        else:
            view = self.env.ref('amazon_ept.amazon_inbound_shipment_print_shipment_label_wizard_form_view', False)
    ...
    def get_unique_package_labels(self):
        ...
        :return: ir.actions.act_window() action
        """
        view = self.env.ref('amazon_ept.amazon_inbound_shipment_print_unique_label_wizard_form_view', False)
    ...
    def check_status(self):
        ...
        for instance, shipment_ids in instance_shipment_ids.items():
            ...
            if response.get('error', {}) and self._context.get('is_auto_process', False):
                self.search_or_create_inbound_common_log_ept('import', response.get('error', {}))
    ...
    def amz_check_status_process_shipments(self, amazon_shipments, instance):
        """
        This method will process amazon shipment and validate pickings in odoo.
        :param amazon_shipments: shipment response from amazon
        :param instance: amazon.instance.ept() object
        :return: boolean(TRUE/FALSE)
        """
        ...
        for ship_member in amazon_shipments:
            flag = False
            cases_required = True if str(ship_member.get('AreCasesRequired', 'false')) == 'true' else False
            ...
            if shipment_status in ['RECEIVING', 'CLOSED']:
                ...
                kwargs.update({'emipro_api': 'check_amazon_shipment_status_spapi',
                               'amazon_shipment_id': shipmentid, 'marketplace_id': instance.market_place_id,})
                response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT + '/iap_request', params=kwargs, timeout=1000)
                if response.get('error', False):
                    raise UserError(_(response.get('error', {})))
    ...
    def amz_prepare_inbound_shipment_kwargs_vals(self, instance):
        """
        This method will Prepare General Arguments for call Amazon SP API.
        :param instance: amazon.instance.ept() object
        :return: kwargs {}
        """
    ...
    def amz_inbound_copy_new_picking(self, response, instance, odoo_shipment_rec,
        ...
        :param amazon_shipment_id:
        :param picking:
        :return: boolean (TRUE/FALSE) or stock.picking()
        """
        picking_obj = self.env['stock.picking']
        new_picking = False
    ...
    def check_status_ept(self, amazon_shipments, seller, job_id=None):
        """
        This method will used to check amazon shipment status.
        :param amazon_shipments: shipment response from amazon
        :param seller: amazon.seller.ept() object
        :param job_id: common.log.book.ept() object
        :return:
        """
        stock_picking_obj = self.env['stock.picking']
        log_obj = self.env['common.log.book.ept']
            ...
            for key, amazon_shipment in amazon_shipments.items():
                ...
                if shipment:
                    ...
                    if pickings:
                        ...
                        if not pickings:
                            ...
                            vals = {
                                ...
                                found for processing  ||| ERP status  : %s """ % (shipmentid, shipment.state),
                                'model_id': log_line_obj.get_model_id('amazon.inbound.shipment.ept'),
                                'res_id': shipment.id,
                    ...
                    vals = {
                        ...
                        'res_id': '', 'log_book_id': job_id.id}
                    log_line_obj.create(vals)
        return True

    def get_remaining_qty_ept(self, instance, amazon_shipment_id, odoo_shipment_rec, items, job_id):
        """
        This method will used to get remaining qty from shipment.
        :param instance: amazon.instance.ept() object
        :param amazon_shipment_id: amazon shipment id
        :param odoo_shipment_rec: amazon.inbound.shipment.ept() object
        :param items: list []
        :param job_id: common.log.book.ept() object
        :return: boolean(TRUE/FALSE)
        """
        ...
        for item in items:
            ...
            done_moves = odoo_shipment_rec.picking_ids.filtered(
                ...
                'move_lines').filtered(
                lambda r, odoo_product=odoo_product: r.state == 'done' and r.product_id.id == odoo_product.id)
    ...
    def update_non_partnered_carrier(self):
        """
        This method will used to put_transport_content_sp_api
        :return: boolean(TRUE/FALSE)
        """
        ctx = self._context.copy() or {}
        auto_called = ctx.get('auto_called', False)
    ...
    def export_partnered_ltl_parcel(self):
        """
        Export shipment data in amazon and if data updated in amazon successfully then update
        shipment as updated_in_amazon = True.
        :return: boolean(TRUE/FALSE)
        """
        for shipment in self:
            log_obj = self.env['common.log.book.ept']
