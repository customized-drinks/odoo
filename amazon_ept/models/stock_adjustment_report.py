...
class StockAdjustmentReportHistory(models.Model):
    ...
    def _compute_get_company(self):
        """
        This method will set the company in stock adjustment report records.
        :return:
        """
        for record in self:
    ...
    def _compute_get_moves_count(self):
        """
        This method will count the number of stock moves in stock adjustment report record.
        :return:
        """
    ...
    mismatch_details = fields.Boolean(compute='_compute_get_log_count', string='Mismatch Details',
                                      help='True if found mismatch details in log lines')
    ...
    def unlink(self):
        """
        This Method will raise UserError if processed or partially processed stock
        adjustment report will be deleted.
        :return:
        """
        for report in self:
    ...
    def default_get(self, fields):
        ...
        if not fields:
            return res
        res.update({'report_type': 'GET_LEDGER_DETAIL_VIEW_DATA'})
        return res

    @api.model
    def create(self, vals):
        """
        Inherited method for set next sequence of stock adjustment report for name of the new report.
        :param vals: dict {}
        :return: amazon.stock.adjustment.report.history() object
        """
    ...
    def list_of_process_logs(self):
        """
        This method will list all Mismatch Details for Stock Adjustment Report.
        :return:
        """
        model_id = self.env['ir.model']._get('amazon.stock.adjustment.report.history').id
        action = {
    ...
    def auto_import_stock_adjustment_report(self, args={}):
        """
        This method will Auto Import Stock Adjustment Reports form amazon in to
        odoo as per given start and end date.
    ...
    def auto_process_stock_adjustment_report(self, args={}):
        """
        This method will auto Process Stock adjustment reports
        :param args: {}
        :return: True
        """
        seller_id = args.get('seller_id', False)
    ...
    def list_of_stock_moves(self):
        """
        This method will open tree view for list stock views for current stock adjustment report.
    ...
    def request_report(self):
        """
        Request GET_LEDGER_DETAIL_VIEW_DATA Report from Amazon for specific
        date range.
        :return: Boolean
        """
        ...
        kwargs.update({'emipro_api': 'create_report_sp_api', 'report_type': self.report_type,
        ...
        if response.get('error', False):
            if self._context.get('is_auto_process', False):
                common_log_book_obj.create({'type': 'import', 'module': 'amazon_ept', 'active': True,
                                            'log_lines': [(0, 0, {'message': 'Inventory Adjustment Report  Process'
                                                                             + response.get('error', {})})]})
                return True
    ...
    def update_report_history(self, request_result):
        """
        This method will Update Report History in odoo
        :param request_result: result of requested report
        :return: boolean(TRUE/FALSE)
        """
        ...
        stock_adjustment_values = {}
        if report_document_id:
            stock_adjustment_values.update({'report_document_id': report_document_id})
        if report_state:
            stock_adjustment_values.update({'state': report_state})
        if report_id:
            stock_adjustment_values.update({'report_id': report_id})
        self.write(stock_adjustment_values)
        return True
    ...
    def process_stock_adjustment_report(self):
        ...
        log_rec = self.get_amazon_stock_adjustment_log()
        create_log = False if self.state == 'partially_processed' else True
        group_wise_lines_list, partially_processed = self._prepare_group_wise_lines_list_ept(log_rec)
        if group_wise_lines_list:
            partially_processed = self._process_group_wise_lines(group_wise_lines_list, log_rec,
    ...
    def get_amazon_stock_adjustment_log(self):
        """
        This method will find existing log book record for current stock adjustment report
        if not found than create new log book record.
        ...
        for row in reader:
            ...
            if not code:
                ...
                job.write({'log_lines': [(0, 0, {
                    'message': 'Code %s configuration not found for processing' % (reason), 'mismatch_details': True
                })]})
                continue
            if len(code.ids) > 1:
                partially_processed = True
            ...
            if not config:
                ...
                job.write({'log_lines': [(0, 0, {
                        'message': 'Seller wise code %s configuration not found for processing' % (
                            code.name), 'mismatch_details': True})]})
                continue
    ...
    def _process_group_wise_lines(self, group_of_data, job, partially_processed, create_log):
        ...
        for config, lines in group_of_data.items():
            ...
            if config.is_send_email:
                # create_email_of_unprocess_lines This Method represents the unprocessed line that creates
                # attachment and sent the attachment to the client.
        ...
        for counter_line in counter_line_list:
            ...
            if counter_vals.get('fulfillment_center_id', '') not in fulfillment_warehouse:
                ...
                if not fn_warehouse or ((counter_vals.get('p_line_disposition', '') != 'SELLABLE' or
                    ...
                    job.write({'log_lines': [(0, 0, {'message': 'mismatch' + message, 'mismatch_details': True})]})
                    continue
            ...
            if exist_move:
                job.write({'log_lines': [
                    (0, 0, {'message': 'Line already processed for Product %s || Code %s' % (
                        product.name, reason), 'product_id': product.id, 'fulfillment_by': 'FBA'})]})
            else:
                vals = self._amz_adjust_prepare_stock_move_vals_ept(product, counter_vals)
                stock_move = stock_move_obj.create(vals)
                stock_move_ids.append(stock_move.id)
    ...
    def _amz_find_fulfillment_center_warehouse(self, line, fulfillment_center_dict, job,
        ...
        if line.get('Fulfillment Center', False) not in fulfillment_center_dict:
            fulfillment_center = fcenter_obj.search(
                [('center_code', '=', line.get('Fulfillment Center', False)),
                 ('seller_id', '=', self.seller_id.id)], limit=1)
            fulfillment_center_dict.update(
                {line.get('Fulfillment Center', False): fulfillment_center or False})
    ...
    def _prepare_counter_line_list(transaction_item_ids, counter_line_list, args):
        ...
        for temp_line in args.get('temp_lines', False):
            if temp_line.get('Reason', '') == args.get('counter_part_code', '') and abs(
                    float(temp_line.get('Quantity', 0.0))) == abs(float(line.get('Quantity', 0.0))) and\
                    temp_line.get('Reference ID', False) not in transaction_item_ids:
                if line.get('Date', '') == temp_line.get('Date', '') and line.get('FNSKU', '') == \
                        temp_line.get('FNSKU', '') and line.get('MSKU', '') == temp_line.get('MSKU', '') and \
                    ...
                    if args.get('create_log', False):
                        job.write({'log_lines': [(0, 0, {'message': message, 'default_code': line.get('MSKU'),
                                                         'fulfillment_by': 'FBA'})]})
                    break
    ...
    def _find_amazon_product_for_process_adjustment_line(self, line, job):
        ...
        if not amazon_product:
            amazon_product = amazon_product_obj.search(
                [('product_asin', '=', asin), ('fulfillment_by', '=', 'FBA')], limit=1)
