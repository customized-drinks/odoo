...
class SaleOrder(models.Model):
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        :return : account.move()
        @author : kishan Sorani on date 30-Sep-2021
        """
    ...
    def import_fba_pending_sales_order(self, seller, marketplaceids, updated_after_date):
        ...
        :param updated_after_date: datetime object
    ...
    def process_fba_pending_sale_order_response(self, seller, kwargs, result):
        ...
        :param kwargs: dict {}
        :param result: fba pending orders response
        :return: boolean(TRUE/FALSE)
        """
    ...
    def cancel_amazon_fba_pending_sale_orders(self, seller, marketplaceids, instance_ids):
        ...
        if not min_draft_order or not max_draft_order:
            if self._context.get('is_auto_process', False):
                log_rec = self.search_or_create_amz_log_ept('sale.order')
                log_rec.write({'log_lines': [(0, 0, {'message': 'No draft order found in odoo'})]})
                return True
        ...
        if not marketplaceids:
            ...
            if not marketplaceids:
                if self._context.get('is_auto_process', False):
                    log_rec = self.search_or_create_amz_log_ept('sale.order')
                    log_rec.write({'log_lines': [(0, 0, {'message': 'There is no any instance is configured of'
    ...
    def cancel_amazon_fbm_pending_sale_orders(self, seller, marketplaceids, instance_ids):
        ...
        :return: boolean(TRUE/FALSE)
        """
        orders = self.search([('amz_seller_id', '=', seller.id), ('amz_fulfillment_by', '=', 'FBM'),
                              ('amz_instance_id', 'in', instance_ids)], order='date_order asc')
        ...
        if unshipped_orders:
            ...
            if not marketplaceids:
                ...
                if not marketplaceids:
                    if self._context.get('is_auto_process', False):
                        log_rec = self.search_or_create_amz_log_ept('sale.order')
                        log_rec.write({'log_lines': [(0, 0, {'message': 'There is no any instance is configured'
                                                                        ' of seller %s' % (seller.name)})]})
                        return True
                    raise UserError(_("There is no any instance is configured of seller %s" % (seller.name)))
            if updated_after_date:
    ...
    def create_amazon_pending_sales_order(self, seller, list_of_wrapper):
        ...
        :param seller: amazon.seller.ept()
        :param list_of_wrapper: list of fba pending orders response
        :return: amazon order list
        """
        common_log_line_obj = self.env['common.log.lines.ept']
        marketplace_instance_dict = {}
        amazon_order_list = []
        ...
        for wrapper_obj in list_of_wrapper:
            ...
            for order in orders:
                ...
                if existing_order:
                    ...
                    continue
                self.request_fba_pending_sale_order_and_process_ept(
                    seller, instance, order, product_details, amazon_order_list, log_rec)
    ...
    def check_amazon_order_vals_ept(self, seller, order, marketplace_instance_dict):
        ...
        :param seller: amazon.seller.ept() object
        :param order: fba order response from amazon
        :param marketplace_instance_dict: marketplace dict {}
        :return: log message, amazon.instance.ept() object
        """
        message = False
        amazon_order_ref = order.get('AmazonOrderId', False)
        if not amazon_order_ref:
    ...
    def request_fba_pending_sale_order_and_process_ept(self, seller, instance, order,
        ...
        :param seller: amazon.seller.ept() object
        :param instance: amazon.instance.ept() object
        :param order: fba pending order response
        :param product_details: amazon product details dict {}
        :param amazon_order_list: amazon order list []
    ...
    def prepare_fba_pending_order_line_ept(order_line):
        ...
        :param row: fba pending order line response
        :param instance: amazon.instance.ept() object
        :param product_details: product details dict {}
        :return: prepare dict {}
        """
    ...
    def prepare_amazon_sale_order_vals(instance, partner_dict, order):
        ...
        if order.get('PurchaseDate', False):
            date_order = parser.parse(order.get('PurchaseDate', False)).astimezone(utc).strftime(
                '%Y-%m-%d %H:%M:%S')
        else:
    ...
    def check_already_status_updated_in_amazon(self, seller, marketplaceids, instances, next_token={}):
        ...
        next_token_wrapper = list_of_wrapper[-1].get('NextToken', '') if list_of_wrapper and isinstance(list_of_wrapper, list) else {}
        ...
        if not seller_sku:
            ...
            if not amazon_product:
                ...
                if odoo_product:
                    ...
                    comman_log_line_obj.amazon_create_order_log_line(
                        message, product_model_id, odoo_product.id, amazon_order_ref,
                        seller_sku, fulfillment_by, log_book)
    ...
    def prepare_updated_ordervals(self, instance, order_ref, order):
        ...
        ordervals = {
            ...
            'auto_workflow_process_id':
                instance.seller_id.fbm_auto_workflow_id.id if instance.seller_id.fbm_auto_workflow_id else False,
    ...
    def amz_create_order_lines(self, order, instance, order_details, dict_product_details):
        ...
        if order.amz_instance_id.is_use_percent_tax:
            ...
            taxargs = {'line_tax_amount_percent': item_tax_percent,
                       'tax_id': [(6, 0, amz_tax_id.id and [amz_tax_id.id] or [])]}
        line_vals = {
            'order_id': order.id,
        ...
        for line in queue_lines:
            ...
            if fulfillment_channel and fulfillment_channel == 'AFN' and \
                ...
                common_log_line_ept.amazon_create_order_log_line(
                    message, model_id, False, amazon_order_ref, False, 'FBM', log_book)
                line.state = 'failed'
                continue
            existing_order = self.search([('amz_order_reference', '=', amazon_order_ref),
                                          ('amz_instance_id', '=', instance.id),
    ...
    def process_shipped_or_missing_unshipped_lines_ept(self, instance, order, line,
        ...
        for order_line_wrapper_obj in list_of_shipped_order_lines:
            ...
            if not isinstance(order_line_wrapper_obj.get('OrderItems', []), list):
                ...
                order_lines = order_line_wrapper_obj.get('OrderItems', [])
            sales_order, line_state = self.process_shipped_or_missing_unshipped_order_ept(
                instance, order, order_lines, line_state, log_book)
            if sales_order:
    ...
    def process_shipped_or_missing_unshipped_order_ept(self, instance, order, order_lines, line_state, log_book):
        ...
        for order_line in order_lines:
            ...
            if not sales_order or sales_order.amz_order_reference != amazon_order_ref:
                ...
                if not partner:
                    message = "Sale Order Skipped because Customer's Address is Missing!"
                    common_log_line_ept.amazon_create_order_log_line(
                        message, model_id, False, amazon_order_ref, False, 'FBM', log_book)
                    line_state = 'failed'
                    continue
                if partner:
                    ...
                    vals.update({'picking_policy': instance.seller_id.fbm_auto_workflow_id.picking_policy or False})
                    ...
                    # update order vals ans set carrier if order status in unshipped
                    # or PartiallyShipped
    ...
    def get_fbm_orders(self, seller, marketplaceids, orderstatus, updated_after_date):
        """
        This method will request for FBM shipped or unshipped amazon orders.
        :param seller: amazon.seller.ept() object
        :param marketplaceids: list amazon marketplcaes ids
        :param orderstatus: fbm orders status
        :param updated_after_date: datetime object
        :return: orders response from sp api
        """
    ...
    def create_shipped_or_missing_unshipped_queue(self, datas, instance_dict, seller, data_queue):
        """
        This method will create an shipped order or missing unshipped orders data queue.
        :param datas: response of orders from sp api
        :param instance_dict: amazon instance dict {}
        :param seller: amazon.seller.ept() object
        :param data_queue: shipped.order.data.queue.ept() object
        :return: boolean(TRUE/FALSE)
        """
        ...
        for data in datas:
            ...
            if not instance:
                instance = seller.mapped('instance_ids').filtered(
                    lambda l: l.marketplace_id.name == data.get('SalesChannel', ''))
    ...
    def import_fbm_shipped_or_missing_unshipped_orders(self, seller, instance, updated_after_date, orderstatus):
        """
        This method process the FBM shipped or unshipped orders.
        :param seller: seller object
        :param instance: instance object
    ...
    def create_amazon_sales_order_vals(self, partner_dict, order, instance):
        ...
        vals.update({'picking_policy': instance.seller_id.fba_auto_workflow_id.picking_policy or False})
