...
class AmazonSellerEpt(models.Model):
    ...
    amz_outbound_instance_id = fields.Many2one(AMAZON_INSTANCE_EPT, string='Default Outbound Instance',
    ...
    def archive_seller(self):
        ...
        if self._context.get('confirm_archive', False):
            ...
            self.env['amazon.product.ept'].search([('instance_id', 'in', self.instance_ids.ids)]).unlink()
    ...
    def amazon_instance_list(self):
        ...
        """
        instance_obj = self.env[AMAZON_INSTANCE_EPT].search([('seller_id', '=', self.id)])
        action = {
            'domain': "[('id', 'in', " + str(instance_obj.ids) + " )]",
            'name': 'Active Instance',
            'view_mode': 'tree,form',
            'res_model': AMAZON_INSTANCE_EPT,
            'type': 'ir.actions.act_window',
        }
        return action
    ...
    def auto_import_fba_shipment_status_ept(self, args={}):
        """
        This method will import fba shipment status from amazon in odoo.
        mod: Action Back Orders Pickings and Create Return Picking
        :param args: {}
        :return:
        """
        ...
        if seller_id:
            ...
            kwargs = {'merchant_id': seller.merchant_id and str(seller.merchant_id) or False,
                      ...
                      'emipro_api': 'check_amazon_shipment_status_spapi',
                      'dbuuid': dbuuid,
            ...
            if amazon_inbound_shipments:
                ...
                status_result = inbound_shipment_obj.check_status_ept(amazon_inbound_shipments, seller, job_id)
                max_range = 0
                ...
                while max_range < len(ship_id_list):
                    ...
                    kwargs = {
                        ...
                        'app_name': 'amazon_ept_spapi',
                        'account_token': account.account_token,
                        'emipro_api': 'check_status_by_shipment_ids',
                        'dbuuid': dbuuid,
                        'amazon_marketplace_code': seller.country_id.amazon_marketplace_code or
                                                   seller.country_id.code,
    ...
    def auto_create_stock_adjustment_configuration(self):
        ...
        for group_id in reason_group_ids:
            ...
            elif group_id.id in [self.env.ref('amazon_ept.amazon_unrecoverable_inventory_ept').id,
                                 self.env.ref('amazon_ept.amazon_transferring_ownership_ept').id]:
                prepare_config_vals.append(
            ...
            elif group_id.id in [self.env.ref('amazon_ept.amazon_software_corrections_ept').id,
                                 self.env.ref('amazon_ept.amazon_catalogue_management_ept').id]:
                prepare_config_vals.append(
    ...
    def migrate_to_sp_api(self):
        ...
        kwargs = {'merchant_id': self.merchant_id and str(self.merchant_id) or False,
                  ...
                  'amazon_marketplace_code': self.country_id.amazon_marketplace_code or
                                             self.country_id.code}
        response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT + '/sp_api_authorisation', params=kwargs, timeout=1000)
    ...
    def request_fba_fulfilment_centers(self, sellers_ids=[]):
        ...
        response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT + '/amazon-fulfillment-center', params=kwargs,
                                         timeout=1000)
