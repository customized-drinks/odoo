...
class InboundShipmentPlanEpt(models.Model):
    ...
    def _compute_odoo_shipment(self):
        """
        This method will count the numbers of inbound shipment
        :return: N/A
        """
        for rec in self:
    ...
    def action_view_inbound_shipment(self):
        """
        This method create and return the action to open the view of inbound shipment
        :return: action
        """
        action = {
    ...
    def create(self, vals):
        """
        This method will update inbound shipment plan name.
        :param vals: dict {}
        :return: inbound.shipment.plan.ept() object
        """
    ...
    def unlink(self):
        """
        Check if Shipment Plan is not in Draft state then it will not Delete
        :return:
        """
        for plan in self:
    ...
    def onchange_instance_id(self):
        """
        This method will set the company and country based on instance
        :return:
        """
        if self.instance_id:
    ...
    def onchange_warehouse_id(self):
        """
        This method will set the ship address based on warehouse
        :return:
        """
    ...
    def reset_all_lines(self):
        ...
        for record in result:
            ...
            plan_line_obj.create({'amazon_product_id': record[0], 'quantity': qty,
                                  'shipment_plan_id': self.id, 'quantity_in_case': quantity_in_case})
        return True

    def set_to_draft_ept(self):
        """
        This method will set inbound shipment plan in draft state.
        :return: boolean(TRUE/FALSE)
        """
    ...
    def _compute_error_logs(self):
        """
        This method will count error logs in inbound shipment plan record.
        :return:
        """
    ...
    def import_product_for_inbound_shipment(self):
        """
        Open wizard to import product through csv file. File contains only product sku and quantity.
        :return:
        """
        ...
        for shipment in odoo_shipments:
            ...
            if not warehouse:
                ...
                error_value = 'No any warehouse found related to fulfillment center %s. Please set ' \
                              'fulfillment center %s in warehouse || shipment %s.' % (fulfill_center,
                                                                                      fulfill_center, shipment.name)
    ...
    def cancel_entire_inbound_shipment(self, shipment, job):
        """
        This method is used to cancel the entire inbound shipment in amazon.
        Note: If we don't get proper response from amazon by using existing flow:
                 ** In CANCEL Shipment in amazon as per User Guide we need to pass sku_qty_list [] and
                    shipping_address_dict {} in SP-API
        :param shipment: inbound shipment record
        :param job: common.log.book.ept() object
        :return: boolean(TRUE/FALSE)
        """
        log_line_obj = self.env['common.log.lines.ept']
    ...
    def cancel_inbound_shipemnts(self, odoo_shipments, job):
        """
        This method used to cancel the inbound shipments in amazon.
        :param odoo_shipments: odoo shipment record
        :param job: common.log.book.ept() object
        :return: boolean(TRUE/FALSE)
        """
    ...
    def display_shipment_details(self, shipments):
        """
        This method will used to display the shipment details
        :param shipments: odoo shipment record
    ...
    def create_inbound_shipment_plan(self):
        ...
        while True:
            ...
            kwargs = {'merchant_id': instance.merchant_id and str(instance.merchant_id) or False,
                      'app_name': 'amazon_ept_spapi',
                      'account_token': account.account_token,
                      'emipro_api': 'create_inbound_shipment_plan_sp_api',
                      'dbuuid': dbuuid,
                      'amazon_marketplace_code': instance.country_id.amazon_marketplace_code or
                                                 instance.country_id.code,
