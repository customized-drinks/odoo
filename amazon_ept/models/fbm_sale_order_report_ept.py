...
class FbmSaleOrderReportEpt(models.Model):
    ...
    mismatch_details = fields.Boolean(compute='_compute_mismatch_details', string='Mismatch Details',
                                      help='True if found any mismatch details in log lines')
    ...
    def create(self, vals):
        """
        This method will set the fbm sale order report name
        :param vals: dict {}
        :return: fbm.sale.order.report.ept() object
        """
    ...
    def default_get(self, fields):
        """
        This method inherited to update the report type
        :param fields:
        :return:
        """
    ...
    def request_report(self):
        ...
        kwargs = self.prepare_fbm_unshipped_data_request_ept('create_amazon_fbm_report_sp_api')
        kwargs.update({'report_type': report_type, 'marketplace_ids': marketplace_ids,
        ...
        if response.get('error', False):
            if self._context.get('is_auto_process', False):
                self.amz_search_or_create_fbm_order_logs_ept(response.get('error', {}))
            else:
    ...
    def amz_search_or_create_fbm_order_logs_ept(self, message):
        """
    ...
    def get_report_request_list(self):
        ...
        if not seller:
            raise UserError(_('Please select Seller'))
        if self.report_id:
            kwargs = self.prepare_fbm_unshipped_data_request_ept('get_report_sp_api')
    ...
    def get_report(self):
        ...
        if self.report_document_id:
            kwargs = self.prepare_fbm_unshipped_data_request_ept('get_report_document_sp_api')
            kwargs.update({'reportDocumentId': self.report_document_id, 'report_id': self.report_id,
                           'amz_report_type': 'fbm_report_spapi', })
            response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT + '/iap_request', params=kwargs, timeout=1000)
            if response.get('error', False):
                if self._context.get('is_auto_process', False):
                    self.amz_search_or_create_fbm_order_logs_ept(response.get('error', {}))
    ...
    def update_report_history(self, request_result):
        ...
        report_state = request_result.get('processingStatus', '_SUBMITTED_')
    ...
    def process_fbm_sale_order_file(self):
        ...
        log_book = self.amz_search_or_create_fbm_order_logs_ept('')
        model_id = self.env['ir.model']._get('fbm.sale.order.report.ept').id
    ...
    def create_amazon_fbm_unshipped_order(self, instance, partner, order_ref, order_details,
                                          business_prime_dict):
        """
    ...
    def prepare_order_dict(order_dict, amz_ref, instance_id, row, business_prime_dict):
        ...
        :param row: file line
        :param business_prime_dict: buyer and recipient name for us region
    ...
    def check_order_exist_in_odoo(self, amz_ref, instance_id):
        ...
        """
        sale_order_obj = self.env['sale.order']
        order = sale_order_obj.search([('amz_instance_id', '=', instance_id),
                                       ('amz_order_reference', '=', amz_ref), ('amz_fulfillment_by', '=', 'FBM')])
        return order
                ...
                if not odoo_product and instance_id.seller_id.create_new_product:
                    ...
                    amazon_product_id = amz_product_obj.create(
                        {'name': odoo_product.name, 'product_id': odoo_product.id,
                         'seller_sku': odoo_product.default_code, 'fulfillment_by': 'FBM',
                         'instance_id': order_ref[1]})
                    message = 'Product is not available in Amazon Odoo Connector and Odoo,' \
                              'So it\'ll created in both.'
                    common_log_line_obj.amazon_create_order_log_line(
                        message, model_id, amazon_product_id, order_ref[0], sku, 'FBM', job)
                elif odoo_product:
                    ...
                    common_log_line_obj.amazon_create_order_log_line(
                        message, model_id, False, order_ref[0], order_detail.get('sku', ''),
                        'FBM', job, mismatch=True)
            else:
    ...
    @staticmethod
    def check_order_line_exist(instance, amz_ref, order_line_ref, sale_order_obj):
        """
    ...
    @staticmethod
    def get_fbm_unshipped_order_line_vals(instance, order, product_id, price_unit, order_detail):
        """
    ...
    @staticmethod
    def prepare_updated_orderline_vals(order_line_vals, order_detail):
        """
