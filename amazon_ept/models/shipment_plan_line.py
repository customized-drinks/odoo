    ...
    def prepare_shipment_plan_line_dict(self, odoo_shipment, items):
        ...
        for item in items:
            ...
            if line and len(line) > 1:
                ...
                if not line:
                    line = ship_plan.shipment_line_ids.filtered(
                        lambda shipment_line: shipment_line.seller_sku == sku and not shipment_line.odoo_shipment_id)
            if line:
            ...
            sku_qty_dict.append({'SellerSKU': sku, 'QuantityShipped': int(qty), 'QuantityInCase': int(quantity_in_case)})
        return sku_qty_dict
