...
class SaleOrderReturnReport(models.Model):
    ...
    def _compute_log_count(self):
        ...
        # Set the boolean field mismatch_details as True is found mismatch details in log lines
    ...
    company_id = fields.Many2one('res.company', string="Company", copy=False,
                                 compute="_compute_return_report_company", store=True)
    log_count = fields.Integer(compute="_compute_log_count", help="Count number of created Stock Move")
    ...
    def default_get(self, fields):
        ...
        if not fields:
            return res
        res.update({'report_type': 'GET_FBA_FULFILLMENT_CUSTOMER_RETURNS_DATA'})
        return res
    ...
    def request_customer_return_report(self):
        """
        Request GET_FBA_FULFILLMENT_CUSTOMER_RETURNS_DATA Report from Amazon for specific date range.
        @author: Keyur Kanani
        :return: Boolean
        """
        model_id = self.env['ir.model']._get('sale.order.return.report').id
        ...
        if response.get('error', {}):
            if self._context.get('is_auto_process', False):
                job = self.get_amz_return_report_log_ept(model_id)
    ...
    def get_amz_return_report_log_ept(self, model_id):
        """
        Define method for search or create amazon return report log.
    ...
    def process_return_report_file(self):
        ...
        for line in reader:
            ...
            return_date = datetime.strftime(parser.parse(line.get('return-date', '')), '%Y-%m-%d %H:%M:%S')
            amazon_order_id = line.get('order-id', '')
            sku = line.get('sku', '')
    ...
    def get_required_moves_for_return_process(self, move_lines, warehouse, return_date, sku, disposition,
        ...
        """
        Define method for prepare stock move and details for return process.
        :param : move_lines :
        :param : warehouse :
        :param : return_date :
        :param : sku :
        ...
        for move in move_lines:
            qty = move.product_qty - sum(move.move_dest_ids.filtered(
                lambda m: m.state in ['partially_available', 'assigned', 'done']).mapped(
    ...
    def auto_import_return_report(self, args={}):
        """
        This method will import customer return report by using scheduler.
        :param args: dict {}
        ...
        if seller_id:
            ...
            return_report.with_context(is_auto_process=True).request_customer_return_report()
    ...
    def auto_process_return_order_report(self, args={}):
        """
        This method will process customer return report by using scheduler.
        :param args: dict {}
        :return:
        """
        seller_id = args.get('seller_id', False)
