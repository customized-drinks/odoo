...
class SettlementReportEpt(models.Model):
    ...
    company_id = fields.Many2one('res.company', string="Company", copy=False,
                                 compute="_compute_settlement_company", store=True)
    start_date = fields.Date()
    end_date = fields.Date()

    already_processed_report_id = fields.Many2one("settlement.report.ept", string="Already Processed Report")
        ...
        for row in settlement_reader:
            ...
            if order_ref in order_name_list and amz_order and row.get('transaction-type', '') == 'Order':
                ...
                order_statement_lines.filtered(
                    lambda l: l.payment_ref == order_line_name and not l.sale_order_id).write({
                ...
                refund_lines.filtered(lambda l, ref_name=ref_name: l.name == ref_name and not l.sale_order_id).write(
                    {'sale_order_id': amz_order.ids[0], 'partner_id': partner.id if partner else False})
    ...
    def reconcile_reimbursement_lines(self, seller, bank_statement, statement_lines, fees_transaction_list):
        ...
        for line in statement_lines:
            ...
            reimbursement_invoice = reimbursement_invoice_ids.filtered(
                lambda l, invoice_type=invoice_type: l.state in ['draft', 'posted'] and not l.payment_state in ['paid', 'in_payment'] and l.move_type == invoice_type)
        ...
        for statement_line_id in statement_lines:
            ...
            if len(invoices) > 1:
                reconcile_invoice = invoices.filtered(
                    lambda l, statement_line=statement_line: round(l.amount_total, 10) == round(statement_line.amount, 10))
            ...
            if len(paid_invoices) > 1:
                paid_invoices = paid_invoices.filtered(
                    lambda l, statement_line=statement_line: round(l.amount_total, 10) == round(statement_line.amount, 10))
            ...
            if unpaid_invoices:
                ...
                for moveline in move_lines:
                    ...
                    amount_currency = 0.0
                    if moveline.amount_currency:
                        currency, amount_currency = self.convert_move_amount_currency(bank_statement, moveline,
    ...
    def reconcile_remaining_transactions(self):
        ...
        for trans_line_id in trans_line_ids:
            ...
            if trans_line_id.id in fees_transaction_list:
                fees_transaction_list[transaction_type_id.amazon_code].append(trans_line_id.id)
            else:
        ...
        for x in range(0, len(statement_lines), 10):
            ...
            for line_id in lines:
                ...
                if trans_line[0].transaction_type_id.is_reimbursement:
                    ...
                    continue
                account_id = trans_line and trans_line[0].account_id.id or self.statement_id.journal_id.payment_credit_account_id.id
                ...
                if tax_id:
                    ...
                    if mv_line_tax_list:
                        ...
                        mv_line_data_list.append({'move_line': {'name': line.name, 'amount': base_amount, 'account_id': account_id, 'tax_tag_ids': mv_line_tag_ids, 'analytic_account_id': analytic_account_id}})
                    ...
                    for move_tax_data in mv_line_tax_list:
                        ...
                        if not vat_account_id:
                            mv_line_data_list = []
                            break
                        mv_line_data_list.append({'vat_line': {'name': vat_mv_line, 'amount': vat_amount, 'account_id': vat_account_id, 'tax_tag_ids': vt_line_tag_ids}})
    ...
    def get_report(self):
        ...
        if self.report_document_id:
            kwargs = self.sudo().prepare_merchant_report_dict(seller, 'get_report_document_sp_api')
            kwargs.update({'reportDocumentId': self.report_document_id, 'name': self.name})
            ...
            if response.get('error', False):
                if self._context.get('is_auto_process', False):
                    job = self.get_amz_settlement_report_log_ept()
    ...
    def get_amz_settlement_report_log_ept(self):
        """
        Define method for search or create amazon settlement report log.
        :return: common.log.book.ept() object
        """
        common_log_book_obj = self.env['common.log.book.ept']
        model_id = self.env['ir.model']._get('settlement.report.ept').id
    ...
    def get_settlement_report_amazon_order_ept(self, row):
        ...
        if fulfillment_by == 'MFN':
            amz_order = sale_order_obj.search([('amz_order_reference', '=', order_ref),
                                               ...
                                               ('amz_fulfillment_by', '=', 'FBM'), ('state', '!=', 'cancel')])
        else:
            domain = [
    ...
    def get_settlement_refund_dict_ept(row, key, product_id, create_or_update_refund_dict):
        ...
        if not create_or_update_refund_dict.get(key, False):
            ...
            existing_amount = create_or_update_refund_dict.get(key, {}).get(product_id, [0.0, 0.0])
    ...
    def check_settlement_report_exist(self, settlement_id):
        ...
        bank_statement_exist = bank_statement_obj.search([('settlement_ref', '=', settlement_id)])
    ...
    def create_settlement_report_bank_statement_line(self, total_amount, settlement_id,
        ...
        if self.instance_id.ending_balance_account_id and float(total_amount) != 0.0:
            ending_payment_ref = '%s/%s/%s' % (settlement_id, self.instance_id.ending_balance_description or
                                               "Ending Balance Description", bank_statement.name)
            bank_statement_line_obj = self.env['account.bank.statement.line']
            bank_line_vals = {
    ...
    def process_settlement_orders(self, bank_statement, settlement_id, orders_list):
        ...
        bank_line_vals = [{'payment_ref': "%s/%s/%s" % (settlement_id, order_key[5], order_key[0]), 'partner_id':
        ...
        for reimbursement_invoice in reimbursement_invoices:
            if reimbursement_invoice.state == 'draft':
                ...
                reimbursement_invoice.with_context(
                    {'check_move_validity': False})._recompute_dynamic_lines(recompute_all_taxes=True)
    ...
    def create_refund_invoices(self, refund_list_item_price, bank_statement):
        ...
        for order_key, product_amount in refund_list_item_price.items():
            ...
            if refund_exist:
                ...
                refund_exist = refund_exist.filtered(
                    lambda l, reserved_refund_invoice_ids=reserved_refund_invoice_ids: l.id not in reserved_refund_invoice_ids)
    ...
    def get_amz_refund_unit_price_ept(invoice_line, amount):
        """
        This method will compute tax and unit price for refund invoice line
        :param invoice_line: account.move.line() object
        :param amount: list [price, tax]
        :return: dict {}
        """
