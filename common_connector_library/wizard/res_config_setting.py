...
class CustomerAppNotificationSettings(models.TransientModel):
    ...
    def enable_emipro_notification(self, notify):
        ...
        send_cron = receive_cron = self.env['ir.cron']
        ...
        try:
            ...
            # receive_cron = self.env.ref("common_connector_library.receive_app_update_notification_ept")
        except Exception as error:
            _logger.error(error)
        if send_cron:
            # if not (is_notify and notify):
            send_cron.write({'active': notify})
            # receive_cron.write({'active': notify})
            if not notify:
